...
The seed also includes examples where guests can attend both days and receive two raffle entries. It also includes one draw where a day 2 prize is won using a day 1 check-in. This is intentional and demonstrates the selected business rule: **one raffle chance per guest per event day, with an event-wide raffle pool**.
